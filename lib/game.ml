  ...
  let print t =
    ...
    let rows_as_strings =
      ...
      if row_num < height -1 then print_endline "-----------------------------")
        ...
        ;;
          let available_captures_for_player (game : t) ~(my_piece : Piece.t) : Move.t list =
    ...
    let _ = make_move_exn ~game:(initial_game) {Move.starting_pos = {Position.row = 0; column = 0}; Move.ending_pos = None} in
    print initial_game;
    [%expect {|
      | O | X | O | X | O | X | O
    -----------------------------
